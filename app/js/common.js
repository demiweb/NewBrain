...
allLozadImg();
$el = $('.header');
$el.toggleClass('header-fixed', $(this).scrollTop() > 70);
...
let bgBlocks = [...document.querySelectorAll('.foundation__cont')];

function blockBg() {
    if (bgBlocks.length) {
        bgBlocks.forEach((word, k) => {
            let trg = word.querySelector('.foundation__bg');
...
let investBlock = document.querySelector('.invest__picture');

function blockInvestAnim() {
    if (investBlock) {
        let trg = investBlock.querySelector('.invest__picture-bg');
        let pt = $(investBlock).css('padding-top');
        let h = window.innerHeight;
        let hMax = $(investBlock).outerHeight(true);
        let wMax = window.innerWidth;
        if (wMax > 1920) {
            wMax = 1920;
        }
        document.body.style.setProperty('--widthEl', `${wMax}px`);
        document.body.style.setProperty('--heightEl', `${hMax}px`);
        // console.log(h);
        var scene2 = new ScrollMagic.Scene({
            triggerElement: investBlock,
            offset: `${(-h / 4) - 10}px`,
            duration: `${h / 2}px`
        })
            .setTween(trg, {
...
let teamBlock = document.querySelector('.our-team__bot');

function blockTeanAnim() {
    if (teamBlock) {
        let trg = teamBlock.querySelector('.our-team__bot-bg');
        let pt = $(teamBlock).css('padding-top');
        let pb = $(teamBlock).css('padding-bottom');
        let h = window.innerHeight;
        let hMax = teamBlock.offsetHeight;
        let wMax = window.innerWidth;
        if (wMax > 1920) {
            wMax = 1920;
        }
